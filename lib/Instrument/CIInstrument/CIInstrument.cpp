...
#include "Instrument/CIInstrument/CIInstrument.h"
...
void CrossInterationInstrument::AddHooksToLoop(vector<Instruction *> & vecInst, ValueToValueMapTy & VMap, ValueToValueMapTy & VCalleeMap, map<Function *, set<Instruction *> > & FuncCallSiteMapping)
{
	...
	for(; itVecBegin != itVecEnd; itVecBegin ++ )
	{
		if((*itVecBegin)->getParent() == pHeader )
		{
			if(LoadInst * pLoad = dyn_cast<LoadInst>(*itVecBegin))
			{
				InlineHookLoad(pLoad, pNewHeader->getTerminator());
			}
			else if(MemTransferInst * pMem = dyn_cast<MemTransferInst>(*itVecBegin))
			{
				InlineHookMem(pMem, pNewHeader->getTerminator());
			}
			else
			{
		...
		else
		{
			if(LoadInst * pLoad = dyn_cast<LoadInst>(*itVecBegin))
			{
				LoadInst * pCloned = cast<LoadInst>(VMap[pLoad]);
				InlineHookLoad(pCloned, pCloned->getParent()->getTerminator());
			}
			else if(MemTransferInst * pMem = dyn_cast<MemTransferInst>(*itVecBegin))
			{
				MemTransferInst * pCloned = cast<MemTransferInst>(VMap[pMem]);
				InlineHookMem(pCloned, pCloned->getParent()->getTerminator());
			}
			else
			{
...
void CrossInterationInstrument::CloneLoopBody(Loop * pLoop, vector<Instruction *> & vecInst )
{
	...
	pLI = &getAnalysis<LoopInfo>(*pFunction);
	pLoop = pLI->getLoopFor(pHeader);
	...
	for(; itSetBlockBegin != itSetBlockEnd; itSetBlockBegin ++ )
	{
		...
		PHINode * pPHI = PHINode::Create(this->LongType, iNum, "", (*itSetBlockBegin)->getFirstInsertionPt());

		for(pred_iterator PI = pred_begin(*itSetBlockBegin); PI != pred_end(*itSetBlockBegin); PI ++ )
		{
			if(setCloned.find(*PI) == setCloned.end() )
			{
				pPHI->addIncoming(this->ConstantLong0, *PI);
			}
			else
			{
				pPHI->addIncoming(this->pAddnumGlobalCounter, *PI);
			}
		}

		pStore = new StoreInst(pPHI, this->numGlobalCounter, "", (*itSetBlockBegin)->getTerminator());
		pStore->setAlignment(8);
	}
