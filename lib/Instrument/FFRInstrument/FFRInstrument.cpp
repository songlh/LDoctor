...
#include "Instrument/FFRInstrument/FFRInstrument.h"
...
void FFRInstrument::CloneFunction(Function * pFunction, vector<BasicBlock *> & vecAdd, ValueToValueMapTy & VMap)
{
	...
	//update branch in the else block
	BasicBlock * pClonedEntry = cast<BasicBlock>(VMap[pRawEntry]);
	...
	for(BasicBlock::iterator II = pRetBlock->begin(); II != pRetBlock->end(); II ++ )
	{
		for(unsigned i = 0; i < II->getNumOperands(); i ++ )
		{
			if(Instruction * pInst = dyn_cast<Instruction>(II->getOperand(i)))
			{
				...
				{
					InstUserMapping[pInst].insert(II);
				}
			}
		}
	}
